...
(defmethod eval-toplevel ::instance-method
  [{:keys [java-type target ^Method method member args]} loader]
  (let [inst (eval-toplevel target loader)
        argvals (into-array Object (map #(eval-toplevel % loader) args))]
    (if method
      (.invoke method inst (box-args (.getParameterTypes method) argvals))
      (Reflector/invokeInstanceMethod inst (name member) argvals))))

(defmethod eval-toplevel ::static-method
  [{:keys [java-type target member method args]} loader]
  (let [argvals (into-array Object (map #(eval-toplevel % loader) args))]
    (if method
      (.invoke method nil (box-args (.getParameterTypes method) argvals))
      (Reflector/invokeStaticMethod target member argvals))))
